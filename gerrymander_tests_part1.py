######################################################################################################################
# Run gerrymander_tests_part1(2012,38,2012,0,0.75,0,'Pennsylvania','foo')
...
def gerrymander_tests_part1(year, states, yearbaseline, statebaseline, imputeduncontested, symm, state_label, outputfilename):
    ...
    parameterlist0 = 'Parameters: year=%i ' %year
    if year==0:
        parameterlist1 = 'states=%s ' % len(states) # need to check!!!!
        parameterlist1 = parameterlist1 + ' yearbaseline=%i statebaseline=' % yearbaseline
    else:
        parameterlist1 = 'Parameters: year=%i states=%i yearbaseline=%i statebaseline=' % (year, len(states), yearbaseline)

    parameterlist2 = '%i ' % len(statebaseline)
    parameterlist3 = 'imputeduncontested=%.2f symm=%i statelabel=%s outputfilename=%s'% (imputeduncontested,symm,state_label,outputfilename)
    ...
    #    results=0
        ...
        if imputeduncontested != 0:
            print('Uncontested races are assumed to have been won with %i%% of the vote.</p>\n<p>' % (imputeduncontested*100), file = f1)

        print('The average Democratic share of the two-party total vote was %2.1f%% (raw)' % (D_mean_raw*100), file = f1)
        ...
        print('If a political party wishes to create for itself an advantage, it will pack its opponents to win overwhelmingly in a small number of districts, while distributing its own votes more thinly, but still to produce reliable wins. ', file = f1)
        print('</p>\n<p></p>\n<p>', file = f1)
        print('Partisan gerrymandering arises not from single districts, but from patterns of outcomes. Thus a single lopsided district may not be an offense - indeed, single-district gerrymandering is permitted by Supreme Court precedent, and may be required for the construction of individual districts that comply with the Voting Rights Act. Rather, it is combinations of outcomes that confer undue advantage to one party or the other.', file = f1)
        print('</p>\n<p></p>\n<p>', file = f1)
        print('The following two tests provide a way of quantifying any such advantage in a set of election results.', file = f1)
        print('</p>\n<p></p>\n<p>', file = f1)
    ...
    #########################################
        ...
        print('To test for a lopsided advantage, one can compare each party''s winning margins and see if they are systematically different. ', file = f1)
        print('This is done using the <a href="http://vassarstats.net/textbook/ch11pt1.html">two-sample t-test</a>. ', file = f1)
        print('In this test, the party with the <i>smaller</i> set of winning margins has the advantage.</p>\n<p></p>\n<p>', file = f1)
    ...
    # mean sample 1 is < mean sample 2, we want P/2< Pcritical, t<0
            p1 = p1 / 2
            if mean(stateresults.tolist()[i] for i in D_districts) > mean(1 - stateresults.tolist()[i] for i in R_districts):
                if t1 < 0:
    ...
    # save the NULL hypothesis
            ...
            if p1>0.05:
                print('The difference between the two parties win margins does not meet established standards for statistical significance. ', file = f1)
                print('The probability that this difference or larger could have arisen by partisan-unbiased mechanisms is %1.2f.' % p1, file = f1)
            else:
                print('The difference between the two parties win margins meets established standards for statistical significance. ', file = f1)
                if p1 >= 0.01:
                    print('The probability that this difference in win margins (or larger) would have arisen by partisan-unbiased mechanisms alone is %1.2f. ' % p1, file = f1)
                else:
                    if p1 >=0.001:
                        print('The probability that this difference in win margins (or larger) would have arisen by partisan-unbiased mechanisms alone is %1.3f. ' % p1, file = f1)
                    else:
                        print('The probability that this difference in win margins (or larger) would have arisen by partisan-unbiased mechanisms alone is less than 0.001. ', file = f1)

            print('</p>\n<p></p>\n<p>', file = f1)

            df_stateraw = stateraw.to_frame()
            df_stateraw['pol'] = np.where(df_stateraw['D_voteshare']>=0.5, 'Democratic', 'Republician')
            df_stateraw['D_voteshare'] = np.where(df_stateraw['D_voteshare']>=0.5, df_stateraw['D_voteshare'] * 100, 100 - df_stateraw['D_voteshare'] * 100)
    ...
    #########################################
        print('%s Test 2\n' % str(datetime.now()))
        print('<b>Second Test of Intents: Probing for asymmetric advantage for one party (mean-median difference and/or chi-square test):</b> ', file = f1)
        ...
        if partisan_balance<0.06:
            print('When the parties are closely matched in overall strength, a partisan advantage will be evident in the form of a difference between the mean (a.k.a. average) vote share and the median vote share, calculated across all districts. </p>\n<p></p>\n<p>', file = f1)
            # mean minus median test
            mean_median_diff = mean(stateresults)-np.median(stateresults)
            SK_mmdiff = mean_median_diff/np.std(stateresults)*np.sqrt(len(stateresults)/0.5708) # the 0.5708 comes from p. 352 of Cabilio and Masaro 1996
            pvalue_mmdiff = min(norm.cdf(SK_mmdiff),1 - norm.cdf(SK_mmdiff)) # One-tailed p-value, usually appropriate since most testers have a direction in mind
            mean_median_diff_p = abs(mean_median_diff) * 100
            if mean_median_diff < 0:
                print('The mean-median difference is %2.1f %% in a direction of advantage to the Democratic Party. ' % mean_median_diff_p, file = f1)
            elif mean_median_diff > 0:
                print('The mean-median difference is %2.1f %% in a direction of advantage to the Republican Party. ' % mean_median_diff_p, file = f1)
            else:
                print('The mean and median are identical, suggesting no identifiable advantage to either major party. This can occur in situations where all races are uncontested.', file = f1)

            pvalue_mmdiff_p = pvalue_mmdiff * 100
            print('The mean-median difference would reach this value in %2.1f %% of situations by a partisan-unbiased process. ' % pvalue_mmdiff_p, file = f1);
